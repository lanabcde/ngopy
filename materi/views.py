...
def kuis_view(request, quiz_id):
    ...
    if request.method == "POST":
        ...
        if score < quiz.kkm :
             ...
             messages.error(request, f"Nilai kamu {score} dari {total_questions} * 5, Kurang dari KKM")
        else: 
                messages.success(request, f"Nilai kamu {score} dari {total_questions}, Selamat!")    
                if quiz_id == 1 :
